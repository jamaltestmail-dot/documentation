...
products with a :ref:`vendor pricelist <purchase/products/pricelist>` and a subcontracting-type
|BoM|. Each component must then be configured with the appropriate route.
...
The contractor (YourCompany) then confirms the *subcontractor* |PO| (2). This creates an |RfQ| to
...
and a :guilabel:`Receipt` smart button appears at the top of the form. In addition, a separate |RfQ|
is automatically created to purchase the components from the dropship vendor.
...
.. image:: subcontracting_dropship/dropship-subcontracting-dsc-order.png
   :alt: Dropship order for the components of a subcontracted product.
