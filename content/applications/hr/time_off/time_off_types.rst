...
as sick time, vacation time, etc. The time off type dictates how the request is made (hours, half
days, or days), and how it is approved.
...
.. note::
   ...
   Type`, the :guilabel:`Duration Type` field, and the :guilabel:`Count as` field. In addition, the
   *Time Off Requests* section **must** be configured.
...
- :guilabel:`Count as`: Using the drop-down menu, select how the time off type is considered in
  terms of accrual plans. Select :guilabel:`Worked Time` if the time off *does* count, select
...
- :guilabel:`Allow Employee Requests`: Enable the checkbox if the employee is able to request more
  time off than was allocated.
...
- :guilabel:`Hide On Dashboard`: Enable this option if time off using this time off type should
...
- :guilabel:`Allow Request on Top`: Enable this checkbox if the employee can request additional time
  off types, stacked on top of this one.

Negative cap section
--------------------

Enable the :guilabel:`Allow Negative Cap` option if employees are able to request more time off than
they currently have, allowing a negative balance. If enabled, an :guilabel:`up to` field appears. In
this field, enter the maximum amount of negative time allowed, in days.
