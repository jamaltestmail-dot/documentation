...
The number of eligible days is set on the employee's profile under the payroll tab, and the time off
type that is considered as the annual leave time off type is set in :menuselection:`Payroll app -->
Configuration --> Settings --> Annual Leave Time-off Type`.
