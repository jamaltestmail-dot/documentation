...
- :guilabel:`Child Support Deduction`: The amount due each pay period for child support.
- :guilabel:`Child Support Garnishee Amount %`: The percentage of pay that is legally withheld and
  paid towards child support.
- :guilabel:`Withhold for Extra Pay`: An additional amount of tax withheld specifically for
  *irregular* payments - such as bonuses, commissions, or back pay.
...
- :guilabel:`Additional Withholding Amount`: An extra fixed amount voluntarily withheld from pay on
  top of normal tax.
...
- :guilabel:`Extra Compulsory Super %`: An additional superannuation percentage that must be
  contributed on top of the standard super guarantee rate.
...
<payroll/payroll_localizations/australia/journal>`, and :ref:`registering payments
<payroll/australia/register>`.

.. _payroll/payroll_localizations/australia/journal:
...
After the :ref:`journal entries <payroll/payroll_localizations/australia/journal>` are validated,
Odoo can generate payments.
...
.. note::
   The |ATO| expects employers to correct errors *within 14 days of detection* or, if the pay cycle
   is longer than 14 days (e.g., monthly), by the date due to lodge the next regular pay event.
...
Terminate Employee`, and a *Terminate Employee* pop-up window loads.
...
The |ATO| :abbr:`API (Application Programming Interfaces)` syncs the journal entries from Employment
