...
.. image:: phone_widget/phone-widget.png
   :alt: Phone widget with four tabs: Keypad, Recent, Contacts, and Activities.
...
settings. If the Odoo database does not have permission to access to the microphone, the
:icon:`fa-microphone` :guilabel:`(microphone)` icon at the top of the widget has a red
:icon:`fa-exclamation-circle` icon :guilabel:`(Microphone access has not been granted yet)`.

Click the icon and select :guilabel:`Click to Activate`, then click :guilabel:`Use Microphone` to
open the browser settings. In the settings, select the option to allow microphone access. When
microphone and speaker access is allowed, the icon has a purple :icon:`fa-cog` icon
:guilabel:`(Audio settings)`.
...
- :icon:`oi-numpad` :guilabel:`Keypad`: Makes :ref:`outbound calls
  ...
  number to search for a contact. Click the :icon:`fa-globe` :guilabel:`(country flag)` icon to fill
  in a country code.
...
- :icon:`fa-address-book` :guilabel:`Contacts`: Lists :doc:`contacts <../../essentials/contacts>`
...
The call activity for the opportunity appears in the :guilabel:`Activities` tab.

.. image:: phone_widget/add-call.png
   :alt: A sales opportunity with a button to add a call to the queue.
...
The call activity for the opportunity disappears from the :guilabel:`Activities` tab.

.. image:: phone_widget/remove-call.png
   :alt: A sales opportunity with a button to remove a call from the queue.
